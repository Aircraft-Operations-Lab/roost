...
class StructuredFlightPlanningProblem(object):
    ...
    @classmethod
    def init_from_nav_network(cls, apm, weather_cache,
                              navigation_network: roost.airspace.NavigationNetwork,
                              setup=None, cconfig=None):
        if cconfig is None:
    ...
    def __init__(self, apm, weather_cache,
        ...
        self.fpe = self.route_graph.generate_flight_plan_encoding(setup['origin'],
                                                                  setup['destination'],
                                                                  cconfig['horizontal_resolution_nm'],
                                                                  climb_descent_coefficients=setup[
                                                                      'climb_descent_profile_coeffs'])
        ...
        max_block = 1024
        blended_dict['fp_vars_per_thread'] = vars_per_thread = int(np.ceil(n_fp_vars * 2 / max_block))
        ...
        blended_dict['t0'] = (blended_dict['departure_time'] - self.wc.start_time) / np.timedelta64(1, 's')
        ...
        self.update_fp_vars = self.get_function('update_fp_vars',
                                                (n_fp_vars, 1), (n_directions_c2, 1, 1), "PPPPPff")

        self.get_objectives = self.get_function('get_objectives',
                                                (n_plans, 1), (n_scenarios, 1, 1), "PPff")

        self.average_objectives = self.get_function('objectives_mean',
                                                    (1, 1), (n_plans_c2, 1, 1), "PP")

        self.clamp_fp_vars = self.get_function('clamp_fp_vars',
                                               (breadth // max_block + 1, 1), (min(breadth, max_block), 1, 1), "PP")

        self.initialize_fp_vars = self.get_function('initialize_fp_vars',
                                                    (n_fp_vars // max_block + 1, 1), (max_block, 1, 1), "PP")

        self.calc_objectives_stdev = self.get_function('objectives_stdev',
                                                       (1, 1), (n_plans_c2, 1, 1), "PP")

        self.generate_rollouts = self.get_function('generate_rollouts',
                                                   (n_directions, 1), (rollout_threads, 2, 1), "PPPPPff")

        self.clear_guidance = self.get_function('clear_guidance',
                                                (n_plans, 1), (4, n_scenarios, 1), "PP")

        self.clear_profiles = self.get_function('clear_profiles',
                                                (n_plans, 36), (n_scenarios, 1, 1), "P")

        self.generate_guidance_matrix = self.get_function('generate_guidance_matrix',
                                                          (n_plans, 1), (n_scenarios, 1, 1), "PPPPPPPP")

        self.generate_4D_profile = self.get_function('generate_4D_profile',
                                                     (n_plans, 1), (n_scenarios, 1, 1), "PPPPPP")

        self.integrate_profile = self.get_function('integrate_profile',
                                                   (n_plans, 1), (n_scenarios, 1, 1), "PPPPi")

        self.create_climb_descent = self.get_function('create_cd_curves',
                                                      (n_plans, 1), (setup['climb_descent_profile_coeffs'], 2, 2), "PP")
        #
        self.evaluate_weather_grib = self.get_function('evaluate_weather_grib_1block', (1, 1), (32, 32, 1), "PPffP")
        self.evaluate_tex = self.get_function('evaluate_tex', (1, 1), (32, 1, 1), "ifffP")
        ...
        self.guidance_int = int_array(n_plans, self.fpe.max_nodes_per_path, n_scenarios, 4)
        self.guidance_float = float_array(n_plans, self.fpe.max_nodes_per_path, n_scenarios, 4)
        self.edge_n_points = a2gpu(self.fpe.edge_n_points, np.int32)
        self.profiles = float_array(n_plans, self.fpe.max_subnodes_per_path, 36, n_scenarios)
        self.edge_subnode_coordinates = a2gpu(self.fpe.ep)
        self.climbdescent_curves = float_array(n_plans, 2, 2, self.pcfg['climb_descent_profile_coeffs'])
        upper_lims = 175 + 50 / 8 * np.array(range(8))
        self.vlims = a2gpu(np.array([[90., 102.85, 112.86, 127.8, 152.8, 175.57, 198.7, 205.], upper_lims]))
        ...
        self.execution_noise_shape = (n_plans, n_scenarios, blended_dict['n_random_vars'])
    ...
    def get_graph(navigation_network, problem_setup):
        ...
        destin_coords = navigation_network.get_point_coords(problem_setup['destination'])
        od_v = destin_coords - origin_coords
        navigation_network = navigation_network.filter_geo([origin_coords - 0.05 * od_v, destin_coords + 0.05 * od_v])
        navigation_network = navigation_network.filter_connected()
        rg = navigation_network.create_graph()
        return rg.reduce_k_shortest_paths(problem_setup['origin'], problem_setup['destination'])
    ...
    def execute_flight_plans(self, i, execution_noise_scaling=1, write_profile=np.int32(0)):
        self.generate_execution_noise()
        if execution_noise_scaling:
            self.execution_noise = .5 + execution_noise_scaling * (self.execution_noise - .5)
        else:
            self.execution_noise.fill(np.float32(0.5))
        # self.guidance_int.fill(0)
        # self.guidance_float.fill(0.0)
        # self.profiles.fill(0.0)
        self.clear_guidance(self.guidance_int.gpudata,
    ...
    def timed_run(self, N=500, initialize=True, noise_scaling=4,
                  blank_steps=50, execution_noise_factor=1, seq_offset=1000, explo_exp=0.5, exec_exp=.5):
        import time
        if initialize:
    ...
    def get_profiles_dataframes(self, clip_to_first_scenario=True):
        ...
        if clip_to_first_scenario:
            dfs = [pd.DataFrame(data=self.profiles.get()[i, :, :35, 0],
                                columns=['Φ', 'λ', 'v_T', 'FL_T', 'χ', 'd2n', 'm', 'rho', 'FL', 'γ', 'TAS', 'TASn',
                                         'gs',
                                         'gsn', 'M', 'Thr', 'D', 't', 'Δt', 'tn', 'fc', 'fcn', 'a', 'p', 'Mg', 'CT',
                                         'eta', 'n_contrail', 'ch4', 'h20','o3', 'contrail_c', 'nox_emission', 'nox_emission_c', 'contrail_dis'])
                   for i in range(self.ccfg['n_plans'])]

            return [dfs[i][dfs[i]['v_T'] > 0]
                    for i in range(self.ccfg['n_plans'])]
        else:
            dfs = [[pd.DataFrame(data=self.profiles.get()[i, :, :35, j],
                                 columns=['Φ', 'λ', 'v_T', 'FL_T', 'χ', 'd2n', 'm', 'rho', 'FL', 'γ', 'TAS', 'TASn',
                                          'gs',
                                          'gsn', 'M', 'Thr', 'D', 't', 'Δt', 'tn', 'fc', 'fcn', 'a', 'p', 'Mg',
                                          'CT',
                                          'eta', 'n_contrail', 'ch4', 'h20','o3', 'contrail_c', 'nox_emission', 'nox_emission_c', 'contrail_dis']) for j in range(self.ccfg['n_scenarios'])]
                   for i in range(self.ccfg['n_plans'])]

            return [[dfs[i][j][dfs[i][j]['v_T'] > 0] for j in range(self.ccfg['n_scenarios'])]
                    for i in range(self.ccfg['n_plans'])]
    ...
    def execute_one_flight_plan(self, m_list, h_list, execution_noise_scaling=1, write_profile=np.int32(0)):
        self.generate_execution_noise()
        self.execution_noise = .5 + execution_noise_scaling * (self.execution_noise - .5)
        # self.guidance_int.fill(0)
        # self.guidance_float.fill(0.0)
        ...
        self.generate_4D_profile(guidance_int_new.gpudata,
                                 self.execution_noise.gpudata,
                                 self.edge_n_points.gpudata,
                                 self.guidance_float.gpudata,
                                 self.edge_subnode_coordinates.gpudata,
